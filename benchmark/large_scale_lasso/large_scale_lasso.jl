...
function model_digest(arrays)
    ctx = SHA.SHA256_CTX()
    update_tag!(ctx, "compact_epigraph_direct_soc")
...
function build_model(data, arrays = model_arrays(data))
    m, n = size(data.A)
    model = Model()
    @variable(model, z[1:length(arrays.c)])
    x = z[1:n]
    u = z[(n + 1):(2n)]
    r = z[end]
    @objective(model, Min, arrays.c' * z)
    @constraint(model, u .>= 0.0)
...
function summary_for(entry::AbstractDict, data, arrays, output_path::AbstractString, elapsed::Real)
    return Dict{String, Any}(
        ...
        "seed" => entry["seed"],
        "nnz" => nnz(data.A),
        "formulation" => "compact_epigraph_direct_soc",
        "variable_count_before_bridge" => 2 * entry["n"] + 1,
        "affine_conic_row_count" => entry["m"] + 2 * entry["n"] + 2,
        "canonical_matrix_nnz" => nnz(data.A) + 4 * entry["n"] + 2,
        "soc_dimension" => entry["m"] + 2,
...
function comparable_result(result::AbstractDict)
    return Dict(
        ...
        for field in (
            ...
            "lambda_bits", "numerical_digest", "model_digest", "formulation",
            "variable_count_before_bridge", "affine_conic_row_count",
            "canonical_matrix_nnz", "soc_dimension",
        )
    )
end
