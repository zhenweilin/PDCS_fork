...
"""
Solve every `.cbf.gz` file in a folder with CuClarabel and cuDSS.

Run with the isolated CuClarabel environment:

    julia --project=benchmark/cuclarabel_env \
        benchmark/multi_period_port_cuclarabel.jl --input_folder /path/to/cbf_files

Each instance gets an isolated raw log. Both `--solver CUCLARABEL` and the
requested spelling `--solver CUCLARABLE` are accepted.
"""

using Dates

const SOLVER_NAME = "CUCLARABEL"
const SOLVER_ALIASES = Set(("cuclarabel", "cuclarable"))
...
function usage(io::IO = stdout)
    ...
    println(
        io,
        "  julia --project=benchmark/cuclarabel_env " *
        "benchmark/multi_period_port_cuclarabel.jl --input_folder PATH [options]",
    )
    println(io)
    println(io, "Options:")
    println(io, "  --input_folder PATH   Folder containing .cbf.gz files (required)")
    println(io, "  --output_folder PATH  Raw-log folder")
    println(io, "  --solver CUCLARABEL   Optional solver-name validation")
...
function parse_arguments(arguments)
    ...
    if haskey(values, "solver")
        normalize_solver(values["solver"]) in SOLVER_ALIASES ||
            error("this script only supports --solver CUCLARABEL")
    end
    ...
    output_folder = abspath(get(
        values,
        "output_folder",
        joinpath(@__DIR__, "results", "multi_period_port", "cuclarabel"),
    ))
...
function build_optimizer(options)
    ...
    if !isnothing(options.time_limit)
        push!(attributes, "time_limit" => options.time_limit)
    end
...
function solve_one(path, options)
    ...
    println("DIRECT_SOLVE_METHOD=cudss")
    ...
    iterations = maybe_get(optimizer, MOI.BarrierIterations(), missing)
