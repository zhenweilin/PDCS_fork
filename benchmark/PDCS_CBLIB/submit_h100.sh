...
input_root="$repo_root/PDCS_CBLIB"
...
find "$input_root" -type f \
    ...
    ! -path '*/hardest_prob/*' -printf '%P\n' | LC_ALL=C sort > "$case_list"
actual_cases="$(wc -l < "$case_list")"
[[ "$actual_cases" == "$expected_cases" ]] || {
    printf 'Expected %s PDCS_CBLIB cases after excluding hardest_prob; found %s\n' \
...
printf '%s\t%s\tPDCS_CBLIB\tcupdcs\t%s\t1e-6\t3600\t%s\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$job_id" "$expected_cases" "$commit" >> "$ledger"
printf 'SUBMITTED dataset=PDCS_CBLIB solver=cupdcs job=%s cases=%s ledger=%s\n' \
