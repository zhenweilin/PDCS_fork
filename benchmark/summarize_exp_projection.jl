...
required = ("cone_count", "input_distribution", "input_sigma", "variant", "strategy",
...
groups = Dict{Tuple{Int,String,String,String,String},Vector{Vector{String}}}()
for row in rows
    row[column["status"]] == "PASS" || continue
    key = (parse(Int, row[column["cone_count"]]), row[column["input_distribution"]],
           row[column["input_sigma"]], row[column["variant"]], row[column["strategy"]])
    push!(get!(groups, key, Vector{Vector{String}}()), row)
end

mkpath(dirname(SUMMARY))
open(SUMMARY, "w") do io
    println(io, "cone_count,cone_dimension,input_distribution,input_sigma,variant,strategy,completed_trials,mean_ms,std_ms,median_ms,min_ms,max_ms,max_error,status")
    for key in sort(collect(keys(groups)); by=x -> (x[1], x[2], parse(Float64, x[3]), x[4], x[5]))
        count, distribution, sigma, variant, strategy = key
        group = groups[key]
        ...
        @printf(io, "%d,3,%s,%s,%s,%s,%d,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,COMPLETE\n",
                count, distribution, sigma, variant, strategy, length(times), mean(times), deviation,
