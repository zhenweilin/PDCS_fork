...
Commands:
  ...
  solve    Run the 15 formal cases (three sizes, five seeds per size).
...
prepare() {
    ...
    read -r formal_count smoke_count < <(
        "${JULIA_BIN}" --startup-file=no --project="${ROOT_DIR}" -e '
            ...
            @assert length(instances) == 15
            @assert length(unique(entry["id"] for entry in instances)) == 15
            for (m, n) in ((100, 5000), (100000, 1000), (250000, 1000))
                @assert count(
