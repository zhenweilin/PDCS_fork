...
function read_and_solve_cbf(file_path::String, output_csv::String, logfile_name::String, kkt_restart_freq::Int64, duality_gap_restart_freq::Int64, use_kkt_restart::Bool, use_duality_gap_restart::Bool)
    ...
    maketask() do task
        ...
        putdouparam(task, MSK_DPAR_OPTIMIZER_MAX_TIME, 3600.0 * 5);
        putdouparam(task, MSK_DPAR_INTPNT_TOL_PFEAS, 1e-6)
        putdouparam(task, MSK_DPAR_INTPNT_TOL_DFEAS, 1e-6)
        putdouparam(task, MSK_DPAR_INTPNT_TOL_REL_GAP, 1e-6)
        numvar = nA
        numcon = m + n
