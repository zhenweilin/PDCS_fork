...
function read_and_solve_cbf(file_path::String, output_csv::String, logfile_name::String, kkt_restart_freq::Int64, duality_gap_restart_freq::Int64, use_kkt_restart::Bool, use_duality_gap_restart::Bool)
    ...
    # @constraint(model, A[1:m+n, :] * x .== b[1:m+n])
    # @constraint(model, A[(m + n + 1) : (m + n + m * n), :] * x .>= b[(m + n + 1) : (m + n + m * n)])
    ...
    # println("scs model: ", model)
    # optimize!(model)
    # println("objective_value: ", objective_value(model))
    # println("x: ", value.(x))
    start_time = time()
    options = Dict{Symbol, Any}()
    options[:time_limit_secs] = 100000
