...
__device__ void dual_heuristic(double *r, double *s, double *t, double *vd1, double *vd2, double *vd3, double *dist){
    ...
    // perspective interior
    if (r[0] > 0.0){
        double td = fmin(t[0], -r[0]*exp(s[0]/r[0]-1.0));
    ...
    *rho = *rho0;
    ...
    if (converged) {
        *rho0 = fmax(*rhol, fmin(*rhoh, *rho));
        ...
        // printf("cuda enter rootsearch_bn_diagonal rho: %f\n", *rho);
...
__global__ void exponent_proj_kernel(double *v, double *t_warm_start, double abs_tol, double rel_tol){
    // heuristic solution
    double r0 = v[0];
    double s0 = v[1];
    double t0 = v[2];
...
__global__ void exponent_proj_diagonal_kernel(double *v, double *D, double *t_warm_start, double abs_tol, double rel_tol){
...
__global__ void dualExponent_proj_diagonal_kernel(double *v, double *D, double *temp, double *t_warm_start, double abs_tol, double rel_tol){
    for (int i = 0; i < 3; ++i) {
        temp[i] = -v[i];
    }
...
__global__ void dualExponent_proj_kernel(double *v, double *t_warm_start, double abs_tol, double rel_tol){
    double vCopy[3];
    for (int i = 0; i < 3; ++i) {
        vCopy[i] = -v[i];
    }
